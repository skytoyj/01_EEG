...
LastEditors: JinYin
LastEditTime: 2023-06-09 15:36:31
FilePath: \08_1024\models\Models.py
Description: 
'''
import torch
...
# EOG: dropout(0.1) EMG: dropout(0.5)
class SimpleCNN(nn.Module):
    def __init__(self, data_num=512):
        super(SimpleCNN, self).__init__()
        self.model = nn.Sequential(
            nn.Conv1d(1, 64, 3, 1, 1), nn.BatchNorm1d(64), nn.ReLU(inplace=True), nn.Dropout(0.5),
            nn.Conv1d(64, 64, 3, 1, 1), nn.BatchNorm1d(64), nn.ReLU(inplace=True), nn.Dropout(0.5),
            nn.Conv1d(64, 64, 3, 1, 1), nn.BatchNorm1d(64), nn.ReLU(inplace=True), nn.Dropout(0.5),
            nn.Conv1d(64, 64, 3, 1, 1), nn.BatchNorm1d(64), nn.ReLU(inplace=True), nn.Dropout(0.5),
        )
